...
<script>
  ...
  const dataTableOptions = (ajaxUrl, start, end, includeUploadButton = false) => ({
    ...
    buttons: [{
      ...
      {
        extend: "excelHtml5",
        text: '<i class="fi fi-rr-file-excel"></i> Excel',
        className: "btn btn-light btn-sm bg-light border me-2 rounded-5"
      },
      {
        extend: "print",
        text: '<i class="fi fi-rr-print"></i> Printo',
        className: "btn btn-light btn-sm bg-light border me-2 rounded-5"
      }
    ],
    columns: [{
  ...
  const table_off_payment_personal = $('#paymentsTable').DataTable(dataTableOptions("api/get_methods/get_complete_invoices.php", '#start_date1', '#end_date1', false));
  ...
  // Refresh table_off_payment_personal on date change
  const refreshTable = table_off_payment_personal => table_off_payment_personal.ajax.reload();
  dateFields.forEach(id => {
    $(id).on('change', () => refreshTable(table_off_payment_personal));
  });
  ...
  $('#clearFiltersBtn').on('click', () => {
    ...
    refreshTable(table_off_payment_personal);
  });
        ...
        $.ajax({
          ...
          success: function(response) {
            if (response.trim() === 'success') {
              ...
              const currentPageSecondTable = tableSecond.page.info().page;
              const currectPaymentPersonal = table_off_payment_personal.page.info().page;
              const currectPaymentBiznes = table_off_payment_biznes.page.info().page;
              // Reload the first table and reset to the previous page
              table.ajax.reload(() => {
                table.page(currentPage).draw(false);
              }, false);
              // Reload the second table and reset to the previous page
              tableSecond.ajax.reload(() => {
                tableSecond.page(currentPageSecondTable).draw(false);
              }, false);
              // Reload the payment personal table and reset to the previous page
              table_off_payment_personal.ajax.reload(() => {
                table_off_payment_personal.page(currectPaymentPersonal).draw(false);
              }, false);
              // Reload the payment biznes table and reset to the previous page
              table_off_payment_biznes.ajax.reload(() => {
                table_off_payment_biznes.page(currectPaymentBiznes).draw(false);
              }, false);
            } else {
              Swal.fire({
