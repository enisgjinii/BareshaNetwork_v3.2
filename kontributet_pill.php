<div class="tab-pane fade show active" id="pills-Kontributet" role="tabpanel" aria-labelledby="pills-Kontributet-tab" tabindex="0">
    <div class="card rounded-5 p-5">
        <form action="add_contribution.php" method="post" enctype="multipart/form-data">
            <div class="row">
                <div class="col mb-3">
                    <label for="" class="form-label">Data e pageses</label>
                    <input type="date" name="date" id="paymentDate" class="form-control rounded-5 border border-1">
                </div>
                <script>
            ...
            <div class="row">
                <div class="col mb-3">
                    ...
                    <select name="periodk" id="periodk" class="form-select rounded-5 border border-1">
            ...
            <div class="row">
                <div class="mb-3">
                    ...
                    <select name="formak" id="formak" class="form-select rounded-5 border border-1">
    ...
    <div class="card rounded-5 p-5">
        ...
        <div class="filter-container mb-3">
            <div class="row">
                <div class="col">
                    <label for="periodFilter" class="form-label">Filtro sipas periodes:</label>
                    <select id="periodFilter" class="form-select">
                        <option value="">All</option>
                        <?php
                        $periods = [];
                        $sql = "SELECT DISTINCT period FROM contributions";
                ...
                <div class="col">
                    <div class="filter-container mb-3">
                        <label for="yearFilter" class="form-label">Filtro :</label>
                        <select id="yearFilter" class="form-select">
                            <option value="">All</option>
                            <?php
                            $years = [];
                            $sql = "SELECT DISTINCT YEAR(date) AS year FROM contributions";
                            $result = $conn->query($sql);
                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    $years[] = $row['year'];
        ...
        <div class="table-responsive">
            <table class="table table-bordered" id="tableOfContributions">
                ...
                <tbody>
                    ...
                    $sql = "SELECT * FROM contributions WHERE 1";
...
<script>
    $(document).ready(function() {
        ...
        $('#yearFilter').on('change', function() {
            table.draw(); // Redraw the table with the custom filter applied
            updateTotalValue(); // Update the total value when the year filter changes
        });
