        ...
        <div class="container-fluid">
            <div class="container">
                <div class="p-5 shadow-sm rounded-5 mb-4 card">
                    ...
                    <nav class="d-flex">
                        <h6 class="mb-0">
                            ...
                            <span>/</span>
                            <a href="pagesat.php" class="text-reset" data-bs-placement="top" data-bs-toggle="tooltip"
                                title="<?php echo __FILE__; ?>"><u>Pagesat e kryera</u></a>
                        </h6>
                    </nav>
                </div>
                ...
                <div class="card shadow-sm rounded-5 my-2 py-5 px-3 mb-4">
                    <form method="GET" action="" class="form-inline">
                        <div class="form-group row">
                            <div class="col-6">
                                Prej :
                                <input type="date" class="form-control shadow-sm rounded-5 w-100 mt-3"
                                    value="<?php echo date("Y-m-d"); ?>" style="width: 230px;" name="d1"
                                    autocomplete="off">
                            </div>
                            <div class="col-6">
                                Deri :
                                <input type="date" class="form-control shadow-sm rounded-5 w-100 mt-3"
                                    value="<?php echo date("Y-m-d"); ?>" style="width: 230px;" name="d2"
                                    autocomplete="off">
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-6">
                                <button type="submit"
                                    class="btn btn-sm btn-primary mt-3 text-white shadow-sm rounded-5 mt-2" name="kerko"
                                    value="Kerko">
                ...
                if (isset($_GET['kerko'])) {
                    $d1 = $_GET['d1'];
                    $d2 = $_GET['d2'];
                    $merri = $conn->query("SELECT * FROM pagesatfacebook WHERE data >= '$d1' AND data <= '$d2' ORDER BY data DESC");
                    ?>
                    <div class="card shadow-sm rounded-5">
                        <div class="card-body">
                            <h4 class="card-title">Tabela e gjeneruar ne baz&euml; t&euml; datave
                                <?php echo $d1; ?> -
                                <?php echo $d2; ?>
                            </h4>
                            <div class="row">
                                            ...
                                            <tbody>
                                                <?php if (mysqli_num_rows($merri) > 0) { ?>
                                                    <?php while ($k = mysqli_fetch_array($merri)) {
                                                        $nrfatures = $k['fatura'];
                                                        $merremrin = $conn->query("SELECT * FROM faturafacebook WHERE fatura='$nrfatures'");
                                                        $dhenat = mysqli_fetch_array($merremrin);
                                                        if ($dhenat != null) {
                                                            $cliidi = $dhenat['emri'];
                                                            $merremrin2 = $conn->query("SELECT * FROM facebook WHERE id='$cliidi'");
                                                            $dhena = mysqli_fetch_array($merremrin2);
                                                        }
                                                        ?>
                                                        <tr>
                                                            <th>
                                                                <?php echo $dhena['emri_mbiemri']; ?>
                                                            </th>
                                                            <td>
                                                                <?php echo $k['fatura']; ?>
                                                            </td>
                                                            <td>
                                                                <?php echo $k['pershkrimi']; ?>
                                                            </td>
                                                            <td>
                                                                <?php echo $k['shuma']; ?>
                                                            </td>
                                                            ...
                                                            <td><a class="btn btn-light shadow-sm rounded-5 border" target="_blank"
                                                                    href="fatura3Facebook.php?invoice=<?php echo $k['fatura']; ?>"><i
                                                                        class="fi fi-rr-print"></i></a></td>
                                                        </tr>
                                                    <?php } ?>
                                                <?php } else { ?>
                                                    <tr>
                                        ...
                                        <tbody>
                                            ...
                                            while ($payment = mysqli_fetch_array($payments)) {
                                                ...
                                                if (!empty($invoice_data)) {
                                                    ...
                                                    ?>
                                                    <tr>
                                                        <td><input type="checkbox" name="selected_payments[]"
                                                                value="<?= $payment['id'] ?>"></td>
                                                        <td>
                                                            <?= $client_data['emri_mbiemri']; ?>
                                                        </td>
                                                        <td>
                                                            <?= $payment['fatura']; ?>
                                                        </td>
                                                        ...
                                                        <td>
                                                            <a class="btn btn-light shadow-2 border border-1" target="_blank"
                                                                href="fatura3Facebook.php?invoice=<?= $payment['fatura']; ?>">
