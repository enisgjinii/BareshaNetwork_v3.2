...
$(document).ready(function () {
    ...
    function trajtoPergigjenMeGabim(textStatus, errorThrown) {
        ...
        Swal.fire({
            ...
            ruajGabimin(errorMessage);
            window.location.href = 'invoice.php?success=error&message=' + encodeURIComponent(errorMessage);
        });
    }

    // Funksioni për ruajtjen e gabimit
    function ruajGabimin(errorMessage) {
        $.ajax({
            url: 'save-error.php',
            type: 'POST',
    ...
    $(document).on('click', '.send-invoice', dergoFaturen('dergofakturen.php', 'Emaili u dërgua me sukses'));
    $(document).on('click', '.send-invoices', dergoFaturen('dergofakturenTekKontabilisti.php', 'Emaili u dërgua me sukses tek kontabilisti'));
});