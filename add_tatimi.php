...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['datetatimi'])) {
        $date = $_POST['datetatimi'];
    }
    if (isset($_POST['text'])) {
        $description = $_POST['text'];
    }
    if (isset($_POST['periodtatimi'])) {
        $period = $_POST['periodtatimi'];
    }
    if (isset($_POST['value'])) {
        $value = $_POST['value'];
    }
    if (isset($_POST['formatatimi'])) {
        $forma = $_POST['formatatimi'];
    }
    ...
    if (isset($_FILES['file'])) {
        if ($_FILES['file']['error'] === UPLOAD_ERR_OK) {
            ...
            $upload_dir = 'tatimi/'; // Directory to store uploaded files
    ...
    if (!empty($date) && !empty($description) && !empty($period) && !empty($value)) {
        ...
        $sql = "INSERT INTO ttatimi (date, description, period, value, document_path, payment_method) VALUES (?, ?, ?, ?, ?, ?)";
