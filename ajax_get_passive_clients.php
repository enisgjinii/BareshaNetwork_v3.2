...
$columns = array('emri', 'aktiv');

// Get the total number of records (without filtering)
$totalRecords = $conn->query("SELECT COUNT(*) as count FROM klientet WHERE aktiv = 1")->fetch_assoc()['count'];
...
if (!empty($searchValue)) {
    $whereClause .= " AND (emri LIKE '%$searchValue%' OR aktiv LIKE '%$searchValue%')";
}

// Perform the query with paging, ordering, and filtering
$query = "SELECT id,emri, aktiv FROM klientet $whereClause ORDER BY $orderBy $orderDir LIMIT $start, $limit";
