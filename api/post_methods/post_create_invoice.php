...
try {
    ...
    if ($type === "grupor") {
        ...
        try {
            ...
            // Optional: Handle case where base_percentage is zero
        ...
        if (insertInvoice($insertStmt, $invoiceData)) {
            // Close the prepared statement and connection
            $insertStmt->close();
            $conn->close();
            ...
            error_log("Error inserting invoice: " . $insertStmt->error);
            throw new Exception("An error occurred while processing the invoice.");
        }
    }
} catch (Exception $e) {
