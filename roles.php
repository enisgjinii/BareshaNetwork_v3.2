    ...
    GROUP BY roles.id';
                    ...
                    <button class="btn btn-danger text-white  rounded-5 delete-row" data-role-id="' . $row['role_id'] . '"><i class="fi fi-rr-trash"></i></button>
                                                ...
                                                if (count($pages) > 5) {
                                                    ...
                                                    echo '</div>';
                                                    echo '<button id="show-more-' . $row['role_id'] . '" class="btn btn-primary show-more rounded-5  text-white" data-role-id="' . $row['role_id'] . '"><i class="fi fi-rr-menu-dots"></i></button>';
                                                } else {
                                                    foreach ($pages as $page) {
                                    ...
                                    GROUP BY googleauth.id, roles.id';
                                        if ($result = $conn->query($sql)) {
                                            while ($row = $result->fetch_assoc()) {
                                                ...
                                                $pages = explode(',', $row['pages']);
                                                foreach ($pages as $page) {
                                                    echo "<div class='col-3 m-1 p-2 bg-white rounded-2 border'>" . format_page_name(trim($page)) . '  </div> <br>';
                                                }
                                                echo '</div></td>';
                                                echo '</tr>';
                                            }
                                ...
                                <div class="container">
                                    <form method="post" action="save_user_role.php">
                                        <div class="form-group">
                                            ...
                                            <select class="form-select  rounded-5" id="user_id" name="user_id">
                                                <?php
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-select  rounded-5" id="role_id" name="role_id">
                                                <?php
                                        ...
                                        <button type="submit" class="btn btn-primary text-white  rounded-5" style="text-transform:none; display: flex; align-items: center;"><i class="fi fi-rr-paper-plane me-2"></i> Ruaj rolin e p&euml;rdoruesit</button>
                                    </form>
                                </div>
                            </div>
                        </div>
